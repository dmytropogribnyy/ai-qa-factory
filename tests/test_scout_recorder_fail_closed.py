...
def test_a_clean_reversible_filter_still_reaches_its_verdict():
    ...
    The page states the filter is now in effect (it offers to clear it), its own facet count
    promises 3 matching items, and the same ten results stay listed. That triple is the defect;
    application without a machine-proven non-matching item is not (see
    tests/test_scout_interaction_oracle.py).
    """
    outcome, _reason = classify(SCENARIO_FILTER,
                                {"result_count": 10, "control_label": "Blue", "facet_count": 3},
                                {"result_count": 10, "control_engaged": True,
