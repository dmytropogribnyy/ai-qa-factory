...
OBSERVER_TOOL_SCHEMAS: List[Dict[str, Any]] = [
    ...
    _schema("observer_list_campaigns", "Read-only paginated list of campaigns with run state + counters (each tagged production/diagnostic), plus the exact build the serving process runs. include_diagnostics=false returns production only.", {**_PAGE, "include_diagnostics": {"type": "boolean"}}),
    _schema("observer_campaign_counts", "Read-only production vs diagnostic canonical campaign counts plus the exact build the serving process runs (matches the Dashboard read model).", {}),
    ...
    _schema("observer_list_findings", "Read-only CANONICAL findings for a campaign: rows are the per-target survivors, each labelled `kind`. `canonical_total` sizes that collection and `offset`/`limit` page it; `total`/`raw_total` are the raw persisted row count and never describe this list. See `count_semantics` in the response.", {**_CAMPAIGN, **_PAGE}, ["campaign_id"]),
    _schema("observer_get_finding", "Read-only single CANONICAL finding — the same labelled row observer_list_findings returns for that id. A row suppressed as a duplicate on its own target is not resolvable here.", {**_CAMPAIGN, "finding_id": {"type": "string"}}, ["campaign_id", "finding_id"]),
