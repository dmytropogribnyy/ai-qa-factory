...
| Internal evidence bundle | runtime | Separate landing/verification screenshots when available, redacted JSON, SHA-256 manifest, and Dashboard/Observer linkage | Reviewer/operator diagnostics; missing capture is reported as missing, never synthesized |
| Client evidence attachment | runtime | Exact-target ≤20 MiB ZIP with offline summary, findings, coverage, available visual evidence, sanitized technical records, and integrity hashes | Completed analysis only; structured text is secret-scanned; screenshots/video require human review |
...
| Manual access-check handoff | conditional | Visible Chromium waits for the operator to complete a CAPTCHA/Cloudflare check, then Continue/Defer/Skip in the same in-memory context | Requires local Playwright/Chromium; session state is not persisted; never solves or bypasses the challenge |
| Dashboard | runtime | Loopback operator UI for stable Overview, active/completed Work, Scout, Activity, Collaboration, Settings/Data & retention, Help, history, target detail, advanced readiness, and guarded lifecycle controls | Local `127.0.0.1`; CSRF/Origin/Host and ownership checks; not a hosted multi-user service |
| Public contact + QA draft | runtime | Up to five source-attributed public emails plus a factual copy-only draft for a completed target with an actionable finding | Nothing is sent; no draft for incomplete/zero-finding runs; implementation offer remains conditional on agreed scope and repo/staging access |
