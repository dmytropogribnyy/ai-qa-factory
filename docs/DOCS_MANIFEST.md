...
| `docs/EMAIL_IDENTITY_AND_MAILBOX_POLICY.md` | Canonical email identities, purposes, OAuth scopes, two-token separation, bounded inbox policy | Yes | integration_added, auth_changed, safety_rule_changed | current | v3.2 — Scout Send (dipptrue, gmail.send) + QA Test Inbox (drdiplextexh, gmail.readonly); asserted by tests/test_v32_docs_consistency.py |
