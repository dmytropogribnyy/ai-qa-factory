...
| `docs/EMAIL_IDENTITY_AND_MAILBOX_POLICY.md` | Canonical email identities, purposes, OAuth scopes, two-token separation, bounded inbox policy | Yes | integration_added, auth_changed, safety_rule_changed | current | v3.2 — Scout Send (dipptrue, gmail.send) + QA Test Inbox (drdiplextech, gmail.readonly); asserted by tests/test_v32_docs_consistency.py |
| `docs/OAUTH_DURABILITY_AND_CREDENTIALS.md` | Gmail Testing-mode 7-day refresh risk + exact owner publish action; GitHub PAT status | Yes | auth_changed, safety_rule_changed | current | v3.2 — owner-only blockers; no secret values |
| `docs/MCP_READINESS_AUDIT_V32.md` | Bounded nine-section MCP readiness audit with real read-only probes | Yes | integration_added | current | v3.2 — Factory plane Declared; agent plane GitHub+Context7 Live Verified |
| `docs/WHOLE_SYSTEM_RELEASE_MATRIX_V32.md` | Whole-system v3.2 release matrix — subsystem → impl/test/CI/screenshot/external-dep + capability class | Yes | phase_completed | current | v3.2 baseline acceptance |
...
| `docs/DASHBOARD_UX_CONTRACT_V1.md` | Binding frontend/backend UX contract — 8-section IA, the 7-tab project/target view, assurance-as-profiles, the ONE canonical read model + envelope, single status vocabularies, client-safe rendering, security invariants and the frontend-writer handoff | Yes | workflow_changed, phase_completed, agent_behavior_changed | current | SUPER MACRO A3 (Issue #74); contract only — resolves the 12 duplications recorded in A2 |
| `docs/DASHBOARD_CURRENT_STATE_V3.md` | Current-state Dashboard truth pack — entry points, full route inventory with exhaustiveness proof, read models and the two-truths Scout seam, operator actions, status vocabularies, client-facing vs operator-only, duplicated UX, and what must be preserved | Yes | workflow_changed, phase_completed | current | SUPER MACRO A2 (Issue #74); description only, no redesign; includes a live 25-route deterministic capture |
| `docs/AI_QA_FACTORY_V3_GAP_ANALYSIS.md` | V3 unified gap analysis — classifies each V3 proposal ALREADY_EXISTS / EXTEND / NEW / DEFER / REJECT against live code, records duplication against the ONE-X invariants, and lists material findings needing owner attention | Yes | phase_completed, workflow_changed | current | SUPER MACRO A1 (Issue #74); design package absent from repo — scope limitation stated in the document |
| `docs/ENGINEERING_EXECUTION_POLICY.md` | Canonical permanent execution policy — truth hierarchy, bounded work, one-writer-per-seam, verification cadence, stationary checkpoints, durable recovery, bounded context, subagents, cost routing, fail-closed/HOLD, client-safe browser tiers, test-flow enablers, honest reporting | Yes | agent_behavior_changed, workflow_changed, safety_rule_changed, phase_completed | current | Materialised from Issue #74 controller/owner decisions (SUPER MACRO A0); points to CLAUDE.md for the fast local loop + tiered CI rather than duplicating them |
