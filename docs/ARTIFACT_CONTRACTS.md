...
New artifact added to `outputs/<project_id>/33_client_audit/`:

| File | Purpose |
|---|---|
| `client_report.md` | Professional, human-readable client QA audit report |

**`client_report.md` characteristics:**
- Generated by `core/reporting/client_delivery_report.py`
- UTF-8 Markdown, 12 sections + Review and Approval footer
- Always contains DRAFT notice and `approved_for_client_delivery = False` statement
- Generated only when `write_files=True`; omitted in `--no-write` runs
- Human-readable language — no raw system log output

**Updated complete artifact list for `33_client_audit/`:**

| File | Description |
|---|---|
| `client_audit_plan.json` | Preflight plan JSON |
| `client_audit_preflight.md` | Preflight plan Markdown |
| `client_audit_run_report.json` | Full run results with structured findings + risk matrix |
| `client_audit_summary.md` | Concise technical summary with Risk Matrix section |
| `client_report.md` | Client-facing audit report (NEW in Phase 6.3) |
| `api_contract_report.json` | API contract analysis (if spec_file provided) |

**New library artifact:**
- `core/reporting/__init__.py` — reporting package
- `core/reporting/client_delivery_report.py` — report generator
