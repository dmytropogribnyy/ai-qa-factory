...
REQUIRED_DOCS: list[tuple[str, str]] = [
    ...
    ("docs/CAPABILITY_MATRIX.md",           "Current runtime capability truth"),
...
def check_current_runtime_claims(root: Path) -> list[CheckResult]:
    """Fail when active operator/client surfaces regress to known stale release claims.

    Historical release notes intentionally retain their original versions. This check is limited to
    files rendered by current CLI, generated client artifacts, or the current capability reference.
    """
    surfaces = {
        "main.py": [
            'help="Prospect QA Scout v1.0',
            'help="Must be 1 in v1.0',
            'print(f"AI QA Factory v5.0.8 completed',
        ],
        "agents/playwright_generator.py": [
            "Generated by AI QA Factory v5.0.8 Model Routing Profiles",
        ],
        "tools/report_builder.py": [
            "# AI QA Factory v5.0.8 Model Routing Profiles — Summary",
        ],
        "docs/CAPABILITY_MATRIX.md": [
            "# Capability Matrix — AI QA Factory v5.0.8",
        ],
        "docs/COMMANDS.md": [
            "## `scout` — Prospect QA Scout v1.0.1",
        ],
        "docs/LIVE_SCOUT_ACCEPTANCE_V33.md": [
            "Actual **short-video recording** runtime",
            "4-job hosted CI on the pushed PR #2 head",
        ],
    }
    results: list[CheckResult] = []
    for rel, stale_fragments in surfaces.items():
        content = read_file(root / rel)
        stale = [fragment for fragment in stale_fragments if fragment in content]
        results.append(make_check(
            doc_path=rel,
            check_type="current_runtime_claim",
            passed=not stale,
            severity="info" if not stale else "error",
            finding=(
                f"CURRENT: {rel} has no known stale runtime claims"
                if not stale
                else f"STALE current-runtime claim(s) in {rel}: {', '.join(stale)}"
            ),
            recommended_action=(
                "" if not stale
                else "Use current version constants and classify runtime/conditional/planning "
                     "capabilities from executable evidence."
            ),
        ))

    matrix = read_file(root / "docs" / "CAPABILITY_MATRIX.md")
    required_truth_terms = (
        ...
        "| generator/planning |",
        "| not runtime |",
        "Automatic reversible business interactions | not runtime",
        "Parallel Scout site execution | not runtime",
    )
    missing = [term for term in required_truth_terms if term not in matrix]
    results.append(make_check(
        doc_path="docs/CAPABILITY_MATRIX.md",
        check_type="capability_status_vocabulary",
        passed=not missing,
        severity="info" if not missing else "error",
        finding=(
            "CURRENT: capability matrix separates runtime, conditional, planning, and unavailable"
            if not missing
            else f"Capability matrix is missing truth-status marker(s): {', '.join(missing)}"
        ),
        recommended_action=(
            "" if not missing
            else "Restore explicit evidence-based status vocabulary and current runtime limits."
        ),
    ))
    readme = read_file(root / "README.md")
    has_current_matrix_link = "docs/CAPABILITY_MATRIX.md" in readme
    results.append(make_check(
        doc_path="README.md",
        check_type="capability_truth_entrypoint",
        passed=has_current_matrix_link,
        severity="info" if has_current_matrix_link else "error",
        finding=(
            "CURRENT: README routes capability claims to the evidence-based matrix"
            if has_current_matrix_link
            else "README does not link the current runtime capability matrix"
        ),
        recommended_action=(
            "" if has_current_matrix_link
            else "Link docs/CAPABILITY_MATRIX.md from the product-scope section."
        ),
    ))
    return results
...
def run_audit(root: Path) -> dict[str, Any]:
    ...
    all_checks += check_current_runtime_claims(root)
